...
/*-------------------------------------------------------------------------*/
char *
gnuplot_get_program_path (char *pname)
{
    int i, j, lg;
    char *path;
...
void
gnuplot_cmd (gnuplot_ctrl * handle, char *cmd, ...)
{
    va_list ap;
...
void
gnuplot_setstyle (gnuplot_ctrl * h, char *plot_style)
{
...
void
gnuplot_set_xlabel (gnuplot_ctrl * h, char *label)
{
...
void
gnuplot_set_ylabel (gnuplot_ctrl * h, char *label)
{
    char cmd[GP_CMD_SIZE];
...
void
gnuplot_resetplot (gnuplot_ctrl * h)
{
    int i;
    if (h->ntmp)
      {
...
void
gnuplot_plot_x (gnuplot_ctrl * handle, double *d, int n, char *title)
{
    int i;
    int tmpfd;
    char name[128];
    ...
    if (title == NULL)
      {
	  sprintf (line, "%s \"%s\" with %s", cmd, name,
		   handle->pstyle);
      }
    else
      {
...
void
gnuplot_plot_xy (gnuplot_ctrl * handle,
		 double *x, double *y, int n, char *title)
{
    int i;
    int tmpfd;
    char name[128];
...
void
gnuplot_plot_slope (gnuplot_ctrl * handle,
		    double a, double b, char *title)
{
...
void
gnuplot_plot_equation (gnuplot_ctrl * h, char *equation, char *title)
{
    char cmd[GP_CMD_SIZE];
